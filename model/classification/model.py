...
def get_model_finetuning_the_convnet(num_classes, pretrained=True):
    """
    ...
    if pretrained:
        model.load_state_dict(torch.load("data/450_act3_mobilenetv3_large.pth"))  # 下载预训练权重
...
def get_model_convnet_as_fixed_feature_extractor(num_classes, pretrained=True):
    """
    获取卷积层固定的resnet18模型
    :param num_classes:
    :param num_classes:
    ...
    in_features = model.linear4.in_features  # 获取全连接层输入种类数
    model.linear4 = nn.Linear(in_features, num_classes)  # 更改全连接层（注意仅最后层的全连接层权重被优化）

    return model


if __name__ == '__main__':
    model = get_model_finetuning_the_convnet(7)
    print(model)
    x = torch.rand((32, 3, 256, 256))
    pred = model(x)
    print(pred.shape)
    model = get_model_convnet_as_fixed_feature_extractor(7)
    print(model)