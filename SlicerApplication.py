...
class SlicerApplication():
    def __init__(self):
        ...
        self.config = self.read_config()  # 定义配置
        self.slicer = ImageSlicer([self.config['image_height'], self.config['image_width']])  # 图片切片器
    ...
    def init_ui(self):
        ...
        self.ui.spinbox_image_height.setValue(self.config['image_height'])  # 切片高
        self.ui.spinbox_image_width.setValue(self.config['image_width'])  # 切片宽
        self.ui.checkbox_drop_last.setChecked(self.config['drop_last'])  # 舍弃边缘
        self.ui.checkbox_enable_filter.setChecked(self.config['enable_filter'])  # 是否过滤
        self.ui.spinbox_threshold.setValue(self.config['threshold'])  # 颜色阈值
        self.ui.lineedit_file_format.setText(self.config['file_format'])  # 文件格式
    ...
    def convert(self):
        ...
        try:
            ...
            self.ui.statusbar.showMessage('转换完成！请打开切片所在文件夹查看切片文件')  # 状态栏
